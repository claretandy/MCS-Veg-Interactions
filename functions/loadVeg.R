...
loadVeg <- function(model.nm="avg5216.4km.std", overwrite=T, doplot=F){
	...
	if (!file.exists(gsub(".pp", ".nc", x=file)) | overwrite==T){
		browser()
		myveg <- readPP(file, outfile=gsub(".pp", ".nc", x=file))
	} else {
		myveg <- brick(gsub(".pp", ".nc", x=file))
	}
