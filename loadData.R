...
mydata <- getMyData(timestep="avg", overwrite=F)
...
# 	myboxes <- makeBoxes(4, myorog) # DON'T USE! 0.5, 1, 2, 4 degree box sizes
	...
	allpatch <- mypatch(threshold=1000, inbr=inbr, id=id[x], land_simple, timestep="avg", indatadir=indatadir, dlresultsdir=dlresultsdir, spp=spp, sppa=sppa, overwrite=F)
...
# 	pop.stats(pop=pop, id=id[x], threshold=1000, timestep="10min", indatadir=indatadir, dlresultsdir=dlresultsdir, overwrite=F)
	...
	mymcsstats <- mcs.stats(mcs=mcs, precip=inbr, id=id[x], threshold=1000, timestep="avg", indatadir=indatadir, dlresultsdir=dlresultsdir, overwrite=F)
